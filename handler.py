...
def update_live_status(sb_client, job_id, status_name, error_msg=None):
    try:
        ...
        print(f"--- [SERVER PRINT]: {status_name} ---")
...
def handler(job):
    ...
    try:
        # 🔥 أول ما السيرفر يصحى من الـ Cold Start ويستلم الطلب، بيبعث إشارة فورية للموقع
        update_live_status(sb_client, job_id, "server_ready")
        print(f"--- [SERVER PRINT]: السيرفر صحي واشتغل الآن بنجاح لـ {job_id} ---")
        ...
        # 🧠 إشارة: السيرفر بدأ بالترجمة والتحسين
        ...
        # 🎨 إشارة: السيرفر بدأ بالتوليد داخل جوجل
        ...
        # 📦 إشارة: السيرفر بدأ بحفظ ورفع الصورة
